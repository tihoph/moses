...
if __name__ == "__main__":
    ...
    bf_pattern = {"csv": "{}", "html": "<b>{}</b>", "latex": r"!bf1! {} !bf2!"}[config.extension]
    it_pattern = {"csv": "{}", "html": "<i>{}</i>", "latex": r"!it1! {} !it2!"}[config.extension]
    ...
    for col, d in zip(targets[1:], directions[1:]):
        metrics[col] = metrics[col].astype(float).round(config.precision)
        sgn = 2 * d - 1
        max_val = sgn * np.max(
            [sgn * m for m, n in zip(metrics[col]["mean"], metrics["Model"]) if n != "Train"]
        )
        metric = [
            format_result(x)
            if x["mean"] != max_val or n == "Train"
            else format_result(x, bf_pattern)
            for (_, x), n in zip(metrics[col].iterrows(), metrics["Model"])
        ]
    ...
    if config.extension == "csv":
        ...
        while i < len(first_header):
            h = first_header[i]
            new_header.append(
                " " * 6 + '<th {}="2">{}{}</th>'.format(spans[i], h, arrow[directions[i]])
            )
            i += 1
            total += 1
            while i < len(first_header) - 1 and first_header[i] == h:
                i += 1
        new_header.extend(["    </tr>", "    <tr>"])
        ...
        latex = latex.split("\n")
        header1 = [
            r"\multirow{2}{*}{" + x.strip() + arrow[d] + "} "
            if "/" not in x
            else r"\multicolumn{2}{c}{" + x.split("/")[0].strip() + arrow[d] + "}"
            for x, d in zip(latex[2].strip()[:-2].split("&"), directions)
        ]
        header1 = [x for i, x in enumerate(header1) if i == 0 or header1[i - 1] != header1[i]]
        header2 = [
            x.split("/")[1].strip() if "/" in x else ""
            for x, d in zip(latex[2].strip()[:-2].split("&"), directions)
        ]
